...
//
// TAQ quotes files are delimiter (using '|' as the delimiter)
// separated text files, using this format:
//
...
//
// TAQ trade files are delimiter (using '|' as the delimiter)
// separated text files, using this format:
//
// timestamp|exchange|ticker|sale condition|volume|price|...
// 093000123456789|K|GOOG|@   |100|900.00|...
// ...
// END|20161024|78721395|||||||||||||||||||||||||
//
// The first line is a header, it defines the fields, each line
// contains all the data for a trade, in this example we are only
// interested in the first few fields, the last line is indicated by
// the 'END' marker, it contains the date (timestamps are relative to
// midnight on this date), and the total number of lines.
