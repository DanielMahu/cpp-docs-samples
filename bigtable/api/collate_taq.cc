...
int main(int argc, char* argv[]) try {
  ...
  // ... this should be define to approximately divide the key space
  // into equal sizes.  The ranges here are not awful, but could be
  // better ...
  char const cuts[] = {'A', 'B', 'D', 'M', 'Q', 'Z' + 1};
...
void append_to_request(bigtable::MutateRowsRequest& request,
                       std::string const& yyyymmdd, char const* column,
                       Message const& msg) {
  ...
  set_cell.set_family_name(column);
  set_cell.set_column_qualifier("timeseries");
  std::string value;
...
void collate_trades(std::shared_ptr<grpc::ChannelCredentials> credentials,
  ...
  // ... then only accept the "event" column ...
  chain.add_filters()->set_column_qualifier_regex_filter("event");

  // ... limit the range of tickers for this search ...
  auto& row_range = *request.mutable_rows()->add_row_ranges();
  row_range.set_start_key_closed(std::string(&begin, 1));
  row_range.set_end_key_open(std::string(&end, 1));
  ...
  while (stream->Read(&response)) {
    for (auto& cell_chunk : *response.mutable_chunks()) {
      ...
      if (cell_chunk.has_qualifier()) {
        ...
        if (current_column != "event") {
          throw std::runtime_error(
              "strange, only 'event' column expected in the query");
        }
      }
      if (cell_chunk.timestamp_micros() != 0) {
        throw std::runtime_error(
