...
Quote parse_taq_quote(int lineno, std::string const& line) try {
...
Trade parse_taq_trade(int lineno, std::string const& line) try {
  // Return the parsed line in this object ...
  Trade trade;
  ...
  // ... store the number of nanoseconds in the timestamp ...
  trade.set_timestamp_ns(ns.count());
  ...
  // Symbol: a string
  std::getline(tokens, tk, '|');  // fetch the symbol
  trade.set_ticker(std::move(tk));

  // Sale Condition: string
  std::getline(tokens, tk, '|');
  trade.set_sale_condition(std::move(tk));
  // Trade Volume: integer
  std::getline(tokens, tk, '|');
  trade.set_qty(std::stol(tk));
  // Trade Price: double
  std::getline(tokens, tk, '|');
  trade.set_px(std::stod(tk));
  // ... the TAQ line has many other fields that we ignore in this demo ...
